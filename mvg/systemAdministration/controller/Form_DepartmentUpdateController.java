...
public class Form_DepartmentUpdateController {
     ...
     *@date : 2018. 9. 20.
     *@author : Sohyoung
     *@Method 설명 : 수정 버튼을 누르면 수정된 부서코드 내용을 DB에 반영하는 메서드
     *@param event
     */
    @FXML
    void click_btn_update(ActionEvent event) {
    	...
    	departmentVo.setDept_code(tf_code.getText());
    	departmentVo.setDept_name(tf_name.getText());
    	
    	service = SystemAdministrationService.getInstance();
    	
    	// 작업 성공 시
    	if(service.updateDepartment(departmentVo) > 0) {
	 ...
	 */
	public void insertToForm(DepartmentVo dept) {
		tf_code.setText(dept.getDept_code());
		tf_name.setText(dept.getDept_name());
	}
