	 ...
	 */
	 ...
	 void ok2(ActionEvent event) {
		 try {
			 ...
			 qtyMap.put("mat_qty",form_tf_mat_qty1.getText());
			 qtyMap.put("mat_code",mat_code );
			 if(mat_qty < Integer.parseInt(form_tf_mat_qty1.getText()) ) {
				 System.out.println("수량 초과");
				 AlertWindow.errorAlert("수량 초과", mat_qty+" 개 이하의 개수를 입력해주세요" );
			 }else {
				 int resutl = houseService.insertAstAdd(assistantVo);
				 if(resutl <= 0) {
					 AlertWindow.FailedAlert((Stage)form_ok.getScene().getWindow());
				 } else {
					 int cnt = houseService.MaterialQty(qtyMap);
					 if(cnt > 0 ) {
						 AlertWindow.SucceedAlert((Stage)form_ok.getScene().getWindow());
					 }
				 }
...
//		 form_tf_mat_date1.setValue(targetDate);
		 form_tf_mat_date1.setValue(LocalDate.now());
		 mat_qty = mat.getMat_qty();
