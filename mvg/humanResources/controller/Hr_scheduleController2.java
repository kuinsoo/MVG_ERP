...
public class Hr_scheduleController2 {
	...
	public void fullCalendarView(YearMonth yearMonth) {
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 7; j++) {
				...
				ap.setStyle("-fx-background-radius: 8px;  -fx-background-color : #ddf1e9; ");
		...
		for (ScheduleEmpVo ap : allCalendarDays) {
			...
			ap.setOnMouseClicked(new EventHandler<Event>() { // 클릭 이벤트 발생
				...
				public void handle(Event event) {
					...
					try {
						FXMLLoader loader= new FXMLLoader(getClass().getResource("../fxml/hr_scheduleNote2.fxml"));
						updateEmp.initModality(Modality.APPLICATION_MODAL); 	// 모달창 셋팅 
						updateEmp.initOwner(t1_tp_main.getScene().getWindow());						// 부모창 설정
						Parent pRoot = loader.load();
						Hr_scheduleNote2Controller formController = loader.getController();
						...
						try {
							...
							FXMLLoader loder = new FXMLLoader(getClass().getResource("../fxml/hr_schedule2.fxml"));// 센터 fxml 주소
			...
			try {
				for (ScheduleEmpVo schVo : schList) {
					if  (ap.getSchemp_date().equals(schVo.getSchemp_date())) {
						ap.setSchemp_note3(schVo.getSchemp_note3());
						ap.setSchemp_note4(schVo.getSchemp_note4());
						if(schVo.getSchemp_note3().equals(null)) {
							note = "";
						} 
						else {
							note = "\n" + schVo.getSchemp_note3();
						}
				...
				txt2.setText(note);
				txt2.setStyle("-fx-fill : blue");

			} catch (Exception e) {
				System.out.println("targerno 2 " + e);
			}
