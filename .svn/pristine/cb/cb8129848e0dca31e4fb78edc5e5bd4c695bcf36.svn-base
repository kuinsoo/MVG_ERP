...
public class DeptOutSrcMovieAddController {
    ...
    void click_btn_add(ActionEvent event) {
    	Stage primaryStage = (Stage)btn_add.getScene().getWindow();
    	try {
    	...
    	schVo.setScd_edate(dp_edate.getValue().toString());
    	schVo.setScd_note(tf_note.getText());
    	int result = outService.addSchedule(schVo);
    	if(result > 0 ) {
    		...
    		AlertWindow.FailedAlert(primaryStage);
    		System.out.println("deptOutSrcMovieAddController " + e);
    	}
    }
